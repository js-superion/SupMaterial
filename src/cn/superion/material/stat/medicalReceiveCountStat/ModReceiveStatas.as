...
import cn.superion.material.stat.medicalReceiveCountStat.view.WinReceiveStatQuery;
...
private function makeExport(dataGridName:Object):void{
	...
	var excelTitle:String='产品入库统计表'+_currentDate;
...
private function addExcelLaster(dataList:Object,fsheet:Sheet):void{   
	...
	for(var j:int=0;j<lary.length;j++){
		if(lary.getItemAt(j).inviteMoney != null && lary.getItemAt(j).inviteMoney != ''){
			initMon7 += lary.getItemAt(j).inviteMoney;
		}
	}
	
	for(var j:int=0;j<lary.length;j++){
		if(lary.getItemAt(j).hecPrice != null && lary.getItemAt(j).hecPrice != ''){
			initMon8 += lary.getItemAt(j).hecPrice;
		}
	}
	for each(var col:* in cols){
		if(i==0){
...
private function preparePrintData(faryData:ArrayCollection):void
{
	for (var i:int=0; i < faryData.length; i++)
	{
		...
		item.salerName=item.salerName == null ? "" : item.salerName;//item.salerName.substr(0,6)
...
protected function printReport(printSign:String):void
{
	...
	if (printSign == '1')
	{
		ReportPrinter.LoadAndPrint("report/material/stat/receiveStatCount.xml", dataList, dict);
	}  
	else
	{
		ReportViewer.Instance.Show("report/material/stat/receiveStatCount.xml", dataList, dict);
	}
}
 ...
 * 差价
 */
private function hecMoney(item:Object, column:DataGridColumn):String
{
	if(item.wholeSaleMoney && item.inviteMoney){
		item.hecPrice = item.inviteMoney - item.wholeSaleMoney;
	}else{
		item.hecPrice = '';
	}
	
	return item.hecPrice;
}