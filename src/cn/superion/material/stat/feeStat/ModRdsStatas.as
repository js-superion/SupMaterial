...
import cn.superion.material.stat.feeStat.view.WinMonth;
import cn.superion.material.stat.feeStat.view.WinRdsStatQuery;
...
private function addExcelLaster(dataList:Object,fsheet:Sheet):void{   
	...
	for each(var col:* in cols){
		if(i==0){
			...
			fsheet.setCell(dataList.dataProvider.length+1,i,initMon11);
		}else if(i==12){
			fsheet.setCell(dataList.dataProvider.length+1,i,initMon12);
		}
		i++;
	}
}
...
protected function printReport(printSign:String):void
{
	...
	if (printSign == '1')
	{
		ReportPrinter.LoadAndPrint("report/material/stat/rdsFeeStat.xml", dataList, dict);
	}
	else
	{
		ReportViewer.Instance.Show("report/material/stat/rdsFeeStat.xml", dataList, dict);
	}
}
/**
 * 拼装打印数据
