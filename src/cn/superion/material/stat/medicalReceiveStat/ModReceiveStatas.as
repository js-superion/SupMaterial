...
import cn.superion.material.stat.medicalReceiveStat.view.WinReceiveStatQuery;
...
protected function printReport(printSign:String):void
{
	...
	if (printSign == '1')
	{
		ReportPrinter.LoadAndPrint("report/material/stat/receiveStatMedical.xml", dataList, dict);
	}  
	else
	{
		ReportViewer.Instance.Show("report/material/stat/receiveStatMedical.xml", dataList, dict);
	}
}
...
{
	if(item.wholeSalePrice && item.invitePrice){
		item.hecPrice = item.invitePrice - item.wholeSalePrice;
	}else{
		item.hecPrice = '';
	}
