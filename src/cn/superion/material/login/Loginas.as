...
import cn.superion.material.util.ObjectInitUtil;
...
private var LicDayBeforeExp:String;
...
protected function init():void
{
	...
	LicDayBeforeExp = ExternalInterface.call("getLicDayBeforeExp");
	currentWin=this;
...
private function submitLogin():void
{
	...
	var ro:RemoteObject=RemoteUtil.getRemoteObject("loginImpl", function(rev:Object):void
	{
		...
		if(LicDayBeforeExp&&LicDayBeforeExp!=""){
			var valDay:String = AppInfo.currentUserInfo.valDay;
			if(valDay&&valDay.length>0 && Number(valDay)<=Number(LicDayBeforeExp)){
				Alert.show('软件许可日期，剩下【'+valDay+'】天','提示');
			}			
		}
...
private function fillUnitsRoles(rev:Object):void
{
	if (rev.data && rev.data.length > 0)
	{
		...
		ObjectInitUtil.unitsList = laryDepList;
