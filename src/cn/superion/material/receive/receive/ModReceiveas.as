...
import cn.superion.material.receive.receive.view.ReceiveOrderFilter;
...
import cn.superion.report.hlib.StringUtil;
...
public static const MENU_NO:String="0201";
//服务类
...
private var isBacthRec:Boolean=false;//采购入库处理时，需要提供物资的库存量批次记录,泰州的为false,东方的为true
...
//	parentDocument.title="采购入库处理";
	...
	isBacthRec=ExternalInterface.call("getIsBacthRec");
...
private function fillIntoGrid(fItem:Array):void
{
	...
	for(var i:int=0;i<fItem.length;i++)
	{
		...
		lnewlDetail.wholeSaleMoney=fItem[i].wholeSalePrice;//byzcl
...
//		lnewlDetail.batch='0'
	...
	if(_bathSign){
		if(batch.text != '1'){//batch.text != '1'  byzcl
			if(isBacthRec){
...
private function makeExport(dataGridName:Object):void{
	...
	var excelTitle:String='采购入库表'+_currentDate;
...
private function preparePrintData(faryData:ArrayCollection):void
{
	...
	for (var i:int=0; i < faryData.length; i++)
	{
		...
		item.className = ArrayCollUtils.findItemInArrayByValue(BaseDict.materialClassDict, 'materialClass', item.materialClass)?
			ArrayCollUtils.findItemInArrayByValue(BaseDict.materialClassDict, 'materialClass', item.materialClass).materialClassName:"";
	}
}
/**
 * 生成表格尾第二行
...
//			"",
//			maker.text)
				lstrLine = "主管:{0}                     采购:{1}                    会计:{2}                   保管员:{3}               制单:{4}             ";
				lstrLine=StringUtils.format(lstrLine, 
					"","","","",maker.text)
//	}
...
//	dict["合计进价"]=lastItem.tradeMoney.toFixed(2);//批发价
	...
	if(stCode != '102'){
		if(isShowLeftLine){
			reportUrl = "report/material/receive/receive2.xml"
		}else{
			reportUrl = "report/material/receive/receive.xml"
		}
	}else{
		if(isShowLeftLine){
			reportUrl = "report/material/receive/YLreceive2.xml"
		}else{
			reportUrl = "report/material/receive/YLreceive.xml"
		}
	}
...
protected function deleteClickHandler(event:Event):void
{
	...
	{
		Alert.show("请先查询要删除的采购入库单！", "提示信息");
		return;
	}

	if (_materialRdsMaster.currentStatus == '1')
	{
		Alert.show("该采购入库单已审核！", "提示信息");
		return;
	}
	else if (_materialRdsMaster.currentStatus == '2')
	{
		Alert.show("该采购入库单已记账！", "提示信息");
		return;
	}
...
protected function verifyClickHandler(event:Event):void
{
	...
	if (_materialRdsMaster.currentStatus == "1")
	{
		Alert.show('该采购入库单已经审核', '提示信息');
		return;
	}

	Alert.show('您是否审核当前采购入库单？', '提示信息', Alert.YES | Alert.NO, null, function(e:*):void
	{
		if (e.detail == Alert.YES)
		{
			var ro:RemoteObject=RemoteUtil.getRemoteObject(DESTANATION, function(rev:Object):void
 ...
 * 过滤采购订单
 * */
private function callbackOrder(e:Event):void{
	var win:ReceiveOrderFilter = PopUpManager.createPopUp(this,ReceiveOrderFilter,true) as ReceiveOrderFilter;
...
private function fillMaster(materialRdsMaster:MaterialRdsMaster):void
{
	...
	invoiceDate.selectedDate = materialRdsMaster.invoiceDate;
...
private function retailPriceLBF(item:Object, column:DataGridColumn):String
{
	...
	if(item.amountPerPackage && item.amountPerPackage!='0'){
		if(_isShow){
			return (item.amountPerPackage * item.retailPrice).toFixed(3);//byzcl
		}else{
			return (item.amountPerPackage * item.retailPrice).toFixed(2);//byzcl
		}
		
	}else{
		if(_isShow){
			return item.retailPrice == null ? "":(item.retailPrice).toFixed(3);
		}else{
...
private function wholeSalePriceLBF(item:Object, column:DataGridColumn):String
{
	...
	if(item.amountPerPackage && item.amountPerPackage!='0'){
		if(_isShow){
			return (item.amountPerPackage * item.wholeSalePrice).toFixed(3);//byzcl
		}else{
			return (item.amountPerPackage * item.wholeSalePrice).toFixed(2);//byzcl
		}
		
	}else{
		if(_isShow){
			return item.wholeSalePrice == null ? "": (item.wholeSalePrice).toFixed(3);
		}else{
...
/**
 * 设置默认打印机的打印页面  byzcl宽210*1000  高288*1000
 * */
private function setPrintPageSize():void{
	...
	lnumHeight=isNaN(lnumHeight)?14*1000:lnumHeight*1000 
