...
private function initToolBar():void
{
	...
	var laryEnables:Array=[toolBar.btQuery,toolBar.btPrint,toolBar.btExcel, toolBar.btSum, toolBar.btExit];
...
private function groupList2(master:MaterialSupplierSummary,lst:ArrayCollection):ArrayCollection{
	...
	for each(var it:Object in lst){
		...
		if(map.deptCode==it.deptCode)
		{
			...
			groupItem.storageName = storageName;
			groupItem.deptName = it.deptName;
			groupItem.detail = detailArray.source;
...
private function groupList(lst:ArrayCollection):ArrayCollection{
	...
	for each(var it:Object in lst){
		...
		temp.deptCode = it.deptCode;
		if(it.deptCode){
			var dept:Object = ArrayCollUtils.findItemInArrayByValue(BaseDict.deptDict,'dept',it.deptCode);
			it.deptName = dept?dept.deptName:"";
		}
	}
	return lst;
}
...
//	}
	...
	for each(var item:Object in tempAry){
		if(item.isSelected){
			...
			materialProvideMaster.sendStatus = '4';
...
//	selAll.selected = false;
	...
	_fparameter.providerName = salerCode.txtContent.text == ''?null:_fparameter.providerName;
